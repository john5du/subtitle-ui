...
- Agent 翻译：`read_subtitle_cues` → `install_translated_cues_preview` → `install_translated_cues`（默认 `label=zh&en`）
- 安全：写操作需 `*_preview` + `confirmToken`；`list_operation_logs` / `rollback_operation` / 备份清理 / 日志清理
- REST 回滚：`POST /api/logs/{id}/rollback`
