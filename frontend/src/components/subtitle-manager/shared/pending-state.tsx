...
export function InlinePending({ label }: { label: string }) {
  return (
    <span className="inline-flex items-center gap-2 rounded-md bg-info-soft px-2.5 py-1 text-xs text-info-muted">
...
export function PanelLoadingOverlay({ label }: { label: string }) {
  return (
    <div className="pointer-events-none absolute inset-0 z-10 flex items-start justify-end p-3">
      <div className="animate-scale-in inline-flex items-center gap-2 rounded-md bg-info-soft px-3 py-1.5 text-xs font-medium text-info-muted">
        <SpinnerIcon className="h-3.5 w-3.5" />
        {label}
      </div>
    </div>
